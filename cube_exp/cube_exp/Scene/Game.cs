...
namespace cube_exp.Scene
{
    class Game : asd.Scene
    {
        ...
        {
            var s1 = BoxObjectFactory.Create<Slime>(new Vector3DI(5, 3, 5), 1);
            s1.SetInitialPosition(new Vector3DI(5, 3, 5));
            s1.IsMaster = false;
            s1.Slave = null;
            layer.AddObject(s1);

            var s2 = BoxObjectFactory.Create<Slime>(new Vector3DI(5, 3, 5), 1);
            s2.SetInitialPosition(new Vector3DI(5, 3, 5));
            s2.IsMaster = false;
            s2.Slave = s1;
            layer.AddObject(s2);

            var s3 = BoxObjectFactory.Create<Slime>(new Vector3DI(5, 3, 5), 1);
            s3.SetInitialPosition(new Vector3DI(5, 3, 5));
            s3.IsMaster = false;
            s3.Slave = s2;
            layer.AddObject(s3);

            Character = BoxObjectFactory.Create<Slime>(new Vector3DI(5, 3, 5), 1);
